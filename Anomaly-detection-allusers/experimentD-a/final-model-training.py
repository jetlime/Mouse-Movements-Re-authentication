...
def createDataset(user):
        ...
        if findSessionLabelIsIllegal(labels, session) == 1:
            ...
            while True:
                ...
                if data[i:i+300].shape[0] < 300 :
                    ...
                    sessions_to_pad_dx.append(list(np.array(data_tmp["dx"])))
                    sessions_to_pad_dy.append(list(np.array(data_tmp["dy"])))
    ...
    paddedsession_dx = keras.preprocessing.sequence.pad_sequences(sessions_to_pad_dx, maxlen=300)
    paddedsession_dy = keras.preprocessing.sequence.pad_sequences(sessions_to_pad_dy, maxlen=300)


    for i in range(0,len(sessions_to_pad_dy)):
        is_legal += 1
        data = pd.DataFrame({"dx":paddedsession_dx[i], "dy":paddedsession_dy[i]})
        X_dataset.append(np.array(data))
        Y_dataset.append(0)
   
    print(Counter(Y_dataset))
    numberoflegalinput = Counter(Y_dataset)[0]
    print(is_illegal)
    randomnegativesamples = getlegalsessionsfromotheruser(user, (2*numberoflegalinput)-is_illegal)
...
for user in users:
    ...
    for fold in range(5):
        ...
        model.summary()
        model.fit(X_train, Y_train, epochs = 400, batch_size =150, verbose = 1, shuffle = True, validation_split=0.1,class_weight=class_weight, callbacks = [tf.keras.callbacks.EarlyStopping(patience=40, verbose=1,restore_best_weights=True, monitor='val_loss', mode='auto'),tf.keras.callbacks.TensorBoard("models-Tensorboard/{}".format(NAME), profile_batch=0),])
        # Save model once training is done.
        filename = str(user) + '-fold' + str(fold+1)
        model_file = os.path.join("models", '{}.h5'.format(filename))
