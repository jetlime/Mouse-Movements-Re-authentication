...
# Obtain illegal data from legal mouse movements of other random users in the dataset
def getIllegalData(user,numberIllegalData):
    global users
    chosenUser = user
    is_legal = 0
    sessions_to_return = []
    X_dataset = []
    # Get as much illegal data as needed
    while is_legal < numberIllegalData:
        # Chose a ranom user
        while chosenUser == user:
            chosenUser = random.choice(users)
        sessions = listdir(base_dir + str(chosenUser))
